...
annot <- data.frame(Ion126 = c(rep("C10", 6), rep("SVEC", 6)),
                    Ion127N = c(rep("C10", 3), rep("SVEC", 6), rep("RAW", 3)),
                    Ion127C = c(rep("SVEC", 6), rep("RAW", 6)),
                    Ion128N = c(rep("SVEC", 3), rep("RAW", 6), rep("C10", 3)),
                    Ion128C = c(rep("RAW", 6), rep("C10", 6)),
                    Ion129N = c(rep("RAW", 3), rep("C10", 6), rep("SVEC", 3)),
                    Ion129C = c(rep("reference", 12)),
                    Ion130N = c(rep("empty", 12)),
                    Ion130C = c(rep("empty", 12)),
                    Ion131N = c(rep("boost", 12)), 
                    ## Note there is a mistake in the table. The author provide 
                    ## the annotation for TMT-10 whereas TMT-11 was used. The 
                    ## last column in the data has little signal, so sample 
                    ## type is assumed to be "empty"
                    Ion131C = c(rep("empty", 12)), 
                    row.names = paste0("Chip_", rep(1:4, each = 3), "_",
                                       rep(c("A", "B", "C"), 4)),
...
expNames <- grep("^Single_Cell", expNames, value = TRUE)
expNames <- sub("(Single.*)_msgf.*$", "\\1", expNames)
...
dats <- lapply(expNames, function(exp){
  ...
  impurities <- read.csv("../../extdata/specht2019/te269088_lot_correction.csv",
  ...
  chip <- sub("^S.*(Chip_\\d_[A-C])$", "\\1", exp)
  pData(dat) <- data.frame(Ion = gsub("Ion_", "", colnames(dat)),
                           ExperimentLabel = exp,
                           Chip = chip,
                           SampleType = unlist(annot[chip, ]),
...
save(dou2019_mouse_peptide, compress = "xz", compression_level = 9,
     file = file.path("../../data/dou2019_mouse_peptide.rda"))
