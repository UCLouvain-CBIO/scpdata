...
acquired using orbitrap-based data-independent acquisition (DIA). The
objective was to demonstrate the capability of their approach by
profiling mouse embryonic stem cell culture conditions, showcasing
heterogeneity in global proteomes, and highlighting differences in the
expression of key metabolic enzymes in distinct cell subclusters.
...
object with 605 assays, each assay being a
...
MassIVE repository `MSV000092429`: ftp://MSV000092429@massive.ucsd.edu/.
...
- **Sample isolation**: Cell sorting was done on a Sony MA900 cell
  sorter using a 130 microm sorting chip. Cells were sorted at
  single-cell resolution, into a 384-well Eppendorf LoBind PCR plate
  (Eppendorf AG) containing 1 microL of lysis buffer.
...
- **Mass spectrometry**: The mass spectrometer was operated in positive
  ...
  MS1 scans were carried out at 120,000 resolution with an automatic
  gain control (AGC) of 300% and maximum injection time set to auto. For
  the DIA isolation window survey, a scan range of 500–900 was used, and
  400–1000 rest of the experiments. Higher energy collisional
  dissociation (HCD) was used for precursor fragmentation with a
  normalized collision energy (NCE) of 33% and the MS2 scan AGC target
  was set to 1000%.
...
The peptide data were formated to a
[SingleCellExperiment::SingleCellExperiment](https://rdrr.io/pkg/SingleCellExperiment/man/SingleCellExperiment.html)
...
The protein data were formated to a
[SingleCellExperiment::SingleCellExperiment](https://rdrr.io/pkg/SingleCellExperiment/man/SingleCellExperiment.html)
object and the sample metadata were matched to the column names and
stored in the `colData`. The object is then added to the
[QFeatures::QFeatures](https://rformassspectrometry.github.io/QFeatures/reference/QFeatures-class.html)
...
**Source article**: Petrosius, V., Aragon-Fernandez, P., Üresin, N. et
al. "Exploration of cell state heterogeneity using single-cell
proteomics through sensitivity-tailored data-independent acquisition."
Nat Commun 14, 5910 (2023). ([link to
article](https://doi.org/10.1038/s41467-023-41602-1)).
...
#> loading from cache
#> An instance of class QFeatures (type: scp) with 605 sets:
#> 
